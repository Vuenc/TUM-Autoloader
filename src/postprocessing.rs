use crate::{GenericResult, data::{CourseFileDownload, CourseVideo, DownloadState, PostprocessingStep}};
use std::process::{Command, Stdio};
use tempfile;
use simple_error::simple_error;


pub fn perform_postprocessing_step(video: &CourseFileDownload<CourseVideo>, step: &PostprocessingStep) -> GenericResult<()> {
    match step {
        PostprocessingStep::FfmpegReencode { target_fps } => {
            ...
            // ffmpeg -i "$file" -filter:v fps=fps=30 -codec:v libx264 -b:v 200k -maxrate:v 200k -bufsize:v 20M "$file-30fps.mp4"
